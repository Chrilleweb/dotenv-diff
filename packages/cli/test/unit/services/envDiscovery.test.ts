...
describe('discoverEnvFiles', () => {
  ...
  it('uses .env.development when it is the only available env file', () => {
    fs.writeFileSync(path.join(cwd, '.env.development'), '');
    ...
    expect(result.envFiles).toEqual(['.env.development']);
